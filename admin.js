...
async function loadAnnouncementBarSettings() {
  try {
    ...
    document.getElementById('ann_messages').value = Array.isArray(d.messages) ? d.messages.join('\n') : '';
  } catch (err) {
    console.error(err);
...
async function saveAnnouncementBar() {
  ...
  btn.disabled = true;
  try {
    const messages = document.getElementById('ann_messages').value
...
function addComponentRow(prefix, data) {
  ...
  row.innerHTML = `
    ...
    <input class="component-nameAr field-input" placeholder="اسم العنصر (مثلاً: عطر شفق)" value="${escapeHTML(data?.nameAr || '')}" />
    <textarea class="component-descAr field-input" rows="2" placeholder="يتكون من... (مثلاً: شجرة العود + زيت الورد)">${escapeHTML(data?.descAr || '')}</textarea>
  `;
