...
async function handleProductFormSubmit(e) {
  ...
  try {
    ...
    if (numericIdForCost !== null) {
      await setDoc(doc(db, 'productCosts', 'product_' + numericIdForCost), { cost: costRaw === '' ? 0 : (parseFloat(costRaw) || 0) }, { merge: true });
    }
    closeProductModal();
    await loadProducts();
  } catch (err) {
...
async function handleCollectionFormSubmit(e) {
  ...
  try {
    ...
    if (numericIdForCost !== null) {
      await setDoc(doc(db, 'productCosts', 'collection_' + numericIdForCost), { cost: costRaw === '' ? 0 : (parseFloat(costRaw) || 0) }, { merge: true });
    }
